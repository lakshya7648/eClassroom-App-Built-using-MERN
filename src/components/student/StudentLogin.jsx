...
const StudentLogin = ({ setProgress, setAlert }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const url = modifyStudentUrl("login");
    setProgress(30);
    let result = {success:false};
    try {
      result = await fetchRequestedData(url, "POST", {"Content-Type":"application/json"}, credentials);
  ...
  useEffect(() => {
    ...
    document.title = "E Class : Student's Login";
  }, []);
      ...
      <div className="w-full h-[88vh] overflow-hidden p-10 bg-gradient-to-br from-white  via-slate-50 to-slate-200 flex flex-wrap justify-center align-middle items-center">
        <div className="title">
          ...
          <p className="block text-lg text-slate-900 font-Young-Serif font-light md:text-xl">
            Join a place where learning is made easy
          </p>
        </div>
